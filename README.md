...
6. **Build, then review it like a product person, not just a linter.** [`product-lens-pr-review`](skills/product-lens-pr-review.md) puts UX first and code second — and every technical finding it posts has been independently verified against the code, with anything that would block a merge forced to survive two agents trying to refute it. It also has a length budget and an explicit `To approve:` contract, because a 13,000-character review teaches people to skim you; and a separate re-review path, because round 3 shouldn't be as long as round 1. [`decision-mining-ledger`](skills/decision-mining-ledger.md) catches the product decisions buried in the implementation logic that a diff alone won't show you.
